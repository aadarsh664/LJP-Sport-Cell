...
import { User, Role, BIHAR_DISTRICTS, DESIGNATIONS, UserStatus } from '../types';
...
import { Language, DISTRICTS_HI, DESIGNATIONS_HI, handleSmartTransliteration } from '../services/translations';
...
export const Directory: React.FC<DirectoryProps> = ({ currentUser, users, onUserClick, t, onUpdateUserStatus, onDeleteUser, onAddMember, onPromoteUser, onAssignBadge, currentLang }) => {
    ...
    const handleTransliterateNewMember = async (field: 'name' | 'fatherName' | 'jurisdiction', sourceScript: 'en' | 'hi') => {
        const sourceField = sourceScript === 'en' ? field : (`${field}_hi` as const);
        const targetField = sourceScript === 'en' ? (`${field}_hi` as const) : field;

        const text = newMember[sourceField as keyof typeof newMember] as string;
        if (text && !newMember[targetField as keyof typeof newMember]) {
            const trans = await handleSmartTransliteration(text, sourceScript);
            setNewMember(prev => ({ ...prev, [targetField]: trans }));
        }
    };
    ...
    const handleAddMemberSubmit = (e: React.FormEvent) => {
        ...
        setNewMember({ name: '', name_hi: '', fatherName: '', fatherName_hi: '', mobile: '', district: BIHAR_DISTRICTS[0], designation: '', jurisdiction: '', jurisdiction_hi: '' });
    };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {filteredUsers.map(user => (
                    <div
                        ...
                        className={`bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-3 md:p-4 relative group transition-all hover:shadow-md cursor-pointer hover:border-ljp-secondary/30`}
                    >
                            ...
                            <div className="flex items-center space-x-3">
                                <div className="relative">
                                    <img
                                        ...
                                        alt={user.name}
                                        className={`w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border-2 ${user.status === UserStatus.SUSPENDED ? 'border-red-500 grayscale' : 'border-gray-100 dark:border-gray-700'}`}
                                    />
                                ...
                                <div>
                                    <h3 className="font-bold text-gray-900 dark:text-white text-sm md:text-base leading-tight flex items-center gap-1">
                                        {currentLang === 'hi' ? (user.name_hi || user.name) : user.name}
                                    ...
                                    <p className="text-[10px] md:text-xs text-ljp-secondary dark:text-blue-400 font-bold uppercase tracking-wide mt-0.5 truncate max-w-[150px]">
                                        {currentLang === 'hi' ? (DESIGNATIONS_HI[user.designation] || user.designation) : user.designation}
                                        {user.jurisdiction ? `, ${currentLang === 'hi' ? (user.jurisdiction_hi || user.jurisdiction) : user.jurisdiction}` : ''}
                                    </p>
                                    <div className="flex items-center text-gray-500 dark:text-gray-400 text-[10px] md:text-xs mt-1">
                                        <MapPin size={10} className="mr-1" />
                                        {currentLang === 'hi' ? (DISTRICTS_HI[user.district] || user.district) : user.district}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-3xl p-6 relative z-10 shadow-2xl animate-fade-in text-center">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 rounded-xl font-bold text-white text-sm shadow-lg transition-transform active:scale-95 ${confirmAction.type === 'delete' ? 'bg-red-500 hover:bg-red-600 shadow-red-200' :
                    ...
                    <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-3xl p-6 relative z-10 shadow-2xl animate-fade-in">
                        ...
                        <form onSubmit={handleAddMemberSubmit} className="space-y-4">
                            <div className="flex gap-2">
                                <input type="text" placeholder={t.namePlaceholder} required className="w-full flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.name} onChange={e => setNewMember({ ...newMember, name: e.target.value })} onBlur={() => handleTransliterateNewMember('name', 'en')} />
                                <input type="text" placeholder="नाम (Hindi)" className="w-full flex-1 bg-blue-50/50 dark:bg-gray-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.name_hi} onChange={e => setNewMember({ ...newMember, name_hi: e.target.value })} onBlur={() => handleTransliterateNewMember('name', 'hi')} />
                            </div>
                            <div className="flex gap-2">
                                <input type="text" placeholder={t.fatherPlaceholder} required className="w-full flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.fatherName} onChange={e => setNewMember({ ...newMember, fatherName: e.target.value })} onBlur={() => handleTransliterateNewMember('fatherName', 'en')} />
                                <input type="text" placeholder="पिता का नाम" className="w-full flex-1 bg-blue-50/50 dark:bg-gray-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.fatherName_hi} onChange={e => setNewMember({ ...newMember, fatherName_hi: e.target.value })} onBlur={() => handleTransliterateNewMember('fatherName', 'hi')} />
                            </div>
                            ...
                            <select className="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" onChange={e => setNewMember({ ...newMember, district: e.target.value })} value={newMember.district}>
                                {BIHAR_DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
                            </select>
                            <select className="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" onChange={e => setNewMember({ ...newMember, designation: e.target.value })} value={newMember.designation} required>
                                <option value="" disabled>{t.designationPlaceholder || "Select Designation"}</option>
                                {DESIGNATIONS.map(d => <option key={d} value={d}>{d}</option>)}
                            </select>

                            <div className="flex gap-2">
                                <input type="text" placeholder="Area/Jurisdiction" className="w-full flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.jurisdiction} onChange={e => setNewMember({ ...newMember, jurisdiction: e.target.value })} onBlur={() => handleTransliterateNewMember('jurisdiction', 'en')} />
                                <input type="text" placeholder="क्षेत्र (Hindi)" className="w-full flex-1 bg-blue-50/50 dark:bg-gray-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-ljp-secondary dark:text-white" value={newMember.jurisdiction_hi} onChange={e => setNewMember({ ...newMember, jurisdiction_hi: e.target.value })} onBlur={() => handleTransliterateNewMember('jurisdiction', 'hi')} />
                            </div>
