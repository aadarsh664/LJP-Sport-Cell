...
import { Language, translations, handleSmartTransliteration } from '../services/translations';
...
export const Auth: React.FC<AuthProps> = ({ onLogin, onSignup, currentLang, onLangChange, totalUsers }) => {
    ...
    const handleTransliterate = async (field: 'name' | 'fatherName' | 'jurisdiction', sourceScript: 'en' | 'hi') => {
        const sourceField = sourceScript === 'en' ? field : (`${field}_hi` as keyof typeof formData);
        const targetField = sourceScript === 'en' ? (`${field}_hi` as keyof typeof formData) : field;

        const text = formData[sourceField as keyof typeof formData] as string;

        if (text && !formData[targetField as keyof typeof formData]) {
            const trans = await handleSmartTransliteration(text, sourceScript);
            setFormData(prev => ({ ...prev, [targetField]: trans }));
        }
    };
        ...
        <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col items-center justify-center p-4 transition-colors duration-300 relative overflow-hidden">
            ...
            <div className="absolute top-6 right-6 flex gap-2 z-10">
                {(['en', 'hi', 'hn'] as Language[]).map(lang => (
                    <button
                        ...
                        className={`text-xs font-bold py-1.5 px-3 rounded-full transition-all border ${currentLang === lang
            ...
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl max-w-md w-full rounded-3xl shadow-2xl overflow-hidden border border-white/20 dark:border-gray-700 z-10 transition-colors">
                <div className="p-6 md:p-8 pb-4 text-center">
                    ...
                    <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{t.appTitle}</h1>
                    <p className="text-ljp-secondary dark:text-blue-400 font-medium text-xs md:text-sm mt-1 uppercase tracking-wide">{t.appSubtitle}</p>
                </div>
                ...
                <div className="px-6 md:px-8 pb-6 md:pb-8 pt-2">
                    ...
                    {!isSignup ? (
                        <form onSubmit={(e) => { e.preventDefault(); onLogin(loginMobile); }} className="space-y-5 animate-fade-in">
                            ...
                            <button className="w-full bg-ljp-primary dark:bg-white text-white dark:text-black py-3 md:py-4 rounded-xl font-bold shadow-lg hover:shadow-xl hover:translate-y-[-2px] transition-all flex items-center justify-center gap-2 group">
                                {t.loginBtn}
                        ...
                        <form onSubmit={handleSignupSubmit} className="space-y-3 animate-fade-in relative">
                            ...
                            <div className="flex gap-2">
                                <input type="text" placeholder={t.namePlaceholder} required className="auth-input flex-1"
                                    value={formData.name}
                                    onChange={e => setFormData({ ...formData, name: e.target.value })}
                                    onBlur={() => handleTransliterate('name', 'en')}
                                    disabled={isSubmitting} />
                                <input type="text" placeholder="नाम (Hindi)" className="auth-input flex-1 bg-ljp-secondary/5 dark:bg-gray-800"
                                    value={formData.name_hi}
                                    onChange={e => setFormData({ ...formData, name_hi: e.target.value })}
                                    onBlur={() => handleTransliterate('name', 'hi')}
                                    disabled={isSubmitting} />
                            </div>

                            <div className="flex gap-2">
                                <input type="text" placeholder={t.fatherPlaceholder} required className="auth-input flex-1"
                                    value={formData.fatherName}
                                    onChange={e => setFormData({ ...formData, fatherName: e.target.value })}
                                    onBlur={() => handleTransliterate('fatherName', 'en')}
                                    disabled={isSubmitting} />
                                <input type="text" placeholder="पिता का नाम" className="auth-input flex-1 bg-ljp-secondary/5 dark:bg-gray-800"
                                    value={formData.fatherName_hi}
                                    onChange={e => setFormData({ ...formData, fatherName_hi: e.target.value })}
                                    onBlur={() => handleTransliterate('fatherName', 'hi')}
                                    disabled={isSubmitting} />
                            </div>
                            ...
                            <div className="flex gap-2">
                                <input type="text" placeholder="Area/Jurisdiction (Optional)" className="auth-input flex-1"
                                    value={formData.jurisdiction}
                                    onChange={e => setFormData({ ...formData, jurisdiction: e.target.value })}
                                    onBlur={() => handleTransliterate('jurisdiction', 'en')}
                                    disabled={isSubmitting} />
                                <input type="text" placeholder="क्षेत्र (Hindi)" className="auth-input flex-1 bg-ljp-secondary/5 dark:bg-gray-800"
                                    value={formData.jurisdiction_hi}
                                    onChange={e => setFormData({ ...formData, jurisdiction_hi: e.target.value })}
                                    onBlur={() => handleTransliterate('jurisdiction', 'hi')}
                                    disabled={isSubmitting} />
                            </div>
