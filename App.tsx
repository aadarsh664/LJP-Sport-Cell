...
import {
    Users, Bell, FileText, ArrowLeft, MoreVertical, Shield,
    Trash2, Ban, CheckCircle, RefreshCw, Star, ArrowRight,
    MapPin, Phone, Award, Upload, AlertTriangle, Calendar,
    MessageSquare, Video, ExternalLink, Activity, Target,
    LandPlot, Clock, User as UserIcon, Check, X, Search, Filter, Globe, Download, Sparkles, UserCircle, BadgeCheck, Edit2
} from 'lucide-react';
import { Language, translations, dynamicTransliterate, fetchTransliteration, handleSmartTransliteration, DESIGNATIONS_HI, DISTRICTS_HI } from './services/translations';
...
export const App: React.FC = () => {
    ...
    const [editProfileData, setEditProfileData] = useState({
        name: '', name_hi: '', mobile: '', district: '', designation: '', fatherName: '', fatherName_hi: ''
    });
    ...
    const handleEnhanceNotice = async (content: string) => {
        if (enhanceCount >= 3) return content;
        try {
    ...
    const handleProcessDownload = async () => {
        ...
        // Wait for render to ensure dynamic text is visible
        await new Promise(resolve => setTimeout(resolve, 300));

        try {
            // Ensure the letterhead is rendered and visible (it's off-screen but rendered)
            const canvas = await toJpeg(letterheadRef.current, {
                quality: 0.95,
                pixelRatio: 2,
    ...
    const handleDownloadIDCard = async () => {
        ...
        try {
            const dataUrl = await toJpeg(idCardRef.current, {
                quality: 0.95,
                pixelRatio: 2,
                backgroundColor: '#ffffff',
                cacheBust: true,
    ...
    const handleTransliterateEditProfile = async (field: 'name' | 'fatherName', sourceScript: 'en' | 'hi') => {
        const sourceField = sourceScript === 'en' ? field : (`${field}_hi` as const);
        const targetField = sourceScript === 'en' ? (`${field}_hi` as const) : field;

        const text = editProfileData[sourceField as keyof typeof editProfileData] as string;
        if (text && !editProfileData[targetField]) {
            const trans = await handleSmartTransliteration(text, sourceScript);
            setEditProfileData(prev => ({ ...prev, [targetField]: trans }));
        }
    };
    ...
    const openEditProfile = () => {
        if (currentUser) {
            setEditProfileData({
                ...
                name_hi: currentUser.name_hi || '',
                mobile: currentUser.mobile,
                district: currentUser.district,
                designation: currentUser.designation,
                fatherName: currentUser.fatherName,
                fatherName_hi: currentUser.fatherName_hi || ''
            });
        ...
        if (viewingNotice) {
            return (
                <div className="p-4 md:p-8 max-w-3xl mx-auto animate-fade-in space-y-6">
                    <button onClick={() => setViewingNotice(null)} className="flex items-center text-gray-500 hover:text-ljp-secondary dark:hover:text-blue-400 font-bold transition-colors mb-2">
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-800">
                        <div className="p-6 md:p-8 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
                            <div className="flex items-center space-x-4">
                                {viewingNotice.userPhoto ? (
                                    <img src={viewingNotice.userPhoto} alt="" className="w-12 h-12 rounded-full object-cover shadow-sm" />
                                ) : (
                                    <div className="w-12 h-12 rounded-full bg-ljp-secondary/10 flex items-center justify-center text-ljp-secondary">
                                        <UserCircle size={28} />
                                    </div>
                                )}
                                <div>
                                    <p className="font-bold text-lg text-gray-900 dark:text-white leading-tight">{viewingNotice.userName}</p>
                                    <p className="text-xs font-bold text-ljp-secondary dark:text-blue-400 uppercase tracking-wide">{viewingNotice.userDesignation}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <span className="bg-red-50 text-red-600 dark:bg-red-900/20 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider inline-flex items-center gap-1 mb-1">
                                    <Bell size={12} /> Official Notice
                                </span>
                                <p className="text-[10px] text-gray-400 font-bold uppercase block">
                                    {new Date(viewingNotice.timestamp).toLocaleDateString()} at {new Date(viewingNotice.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </p>
                            </div>
                        </div>

                        <div className="p-6 md:p-8 text-gray-800 dark:text-gray-200 whitespace-pre-wrap leading-relaxed text-base md:text-lg">
                            {viewingNotice.content}
                        </div>

                        <div className="p-4 md:p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 flex justify-end">
                            <button
                                onClick={() => handleDownloadLetterhead(viewingNotice)}
                                className="flex items-center space-x-2 px-5 py-2.5 bg-ljp-secondary text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-200 dark:shadow-none hover:brightness-110 transition-all"
                            >
                                <Download size={18} />
            ...
            case 'home':
                return (
                    <Dashboard
                        ...
                        onViewNotice={handleNoticeClick}
                    ...
                    return (
                        <div className="animate-fade-in space-y-6">
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700 relative">
                                <div className="h-24 md:h-40 bg-gradient-to-r from-ljp-secondary to-black dark:from-blue-900 dark:to-gray-900"></div>
                                <div className="px-8 relative">
                                    <div className="absolute -top-12 md:-top-20">
                                        <div className="relative inline-block">
                                            <img src={selectedUser.photoUrl || "https://picsum.photos/150/150"} className="w-24 h-24 md:w-40 md:h-40 rounded-full border-4 border-white dark:border-gray-800 shadow-lg object-cover bg-gray-200" alt={selectedUser.name} />
                                            {selectedUser.badge && (
                                ...
                                <div className="mt-14 md:mt-24 px-4 md:px-8 pb-6 md:pb-10">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h2 className="text-xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-2 flex-wrap">
                                                {selectedUser.name}
                                            ...
                                            <p className="text-ljp-secondary dark:text-blue-400 font-bold text-base md:text-lg break-words">{selectedUser.designation}</p>
                                            <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">{t.memberSince} {calculateTenure(selectedUser.joinedDate)}</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 mt-5 md:mt-8">
                                        <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                            <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">{t.targetDistrict}</label>
                                            <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words"><MapPin size={20} className="mr-2 text-ljp-accent shrink-0" /> {selectedUser.district}</p>
                                        </div>
                                        ...
                                        {isPrivileged ? (
                                            <>
                                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                                    ...
                                                    <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words"><Phone size={20} className="mr-2 text-ljp-accent shrink-0" /> {selectedUser.mobile}</p>
                                                </div>
                                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                                    <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Father's Name</label>
                                                    <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words"><UserIcon size={20} className="mr-2 text-ljp-accent shrink-0" /> {selectedUser.fatherName}</p>
                                                </div>
                                            </>
                                        ) : (
                                            <>
                                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 opacity-60">
                                                    <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Mobile</label>
                                                    <p className="font-semibold flex items-center text-gray-500 dark:text-gray-400 text-sm md:text-lg italic break-words"><Phone size={20} className="mr-2 shrink-0" /> Hidden</p>
                                                </div>
                                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 opacity-60">
                                                    <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Father's Name</label>
                                                    <p className="font-semibold flex items-center text-gray-500 dark:text-gray-400 text-sm md:text-lg italic break-words"><UserIcon size={20} className="mr-2 shrink-0" /> Hidden</p>
                                                </div>
                                            </>
                                        )}
                                        ...
                                        <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                            ...
                                            <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words">
                                                <LandPlot size={20} className="mr-2 text-ljp-accent shrink-0" />
                        ...
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                            {(['All', 'State Level', 'District Level'] as const).map(filter => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${meetingFilter === filter
                ...
                if (currentUser.role === Role.SUPER_ADMIN || currentUser.role === Role.SUB_ADMIN) {
                    ...
                    return (
                        <div className="space-y-6 animate-fade-in">
                            ...
                            <div onClick={() => { setSelectedUser(currentUser); setCurrentTab('directory'); }} className="bg-gradient-to-r from-ljp-secondary to-black dark:from-gray-800 dark:to-gray-900 p-6 rounded-3xl shadow-lg border border-white/10 cursor-pointer hover:scale-[1.01] transition-transform">
                                <div className="flex items-center space-x-5">
                                    ...
                                    <div>
                                        <p className="font-bold text-xl text-white mb-1">{currentLang === 'hi' ? (currentUser.name_hi || currentUser.name) : currentUser.name}</p>
                                        <p className="text-white/70 font-medium">{currentLang === 'hi' ? (DESIGNATIONS_HI[currentUser.designation] || currentUser.designation) : currentUser.designation}</p>
                                            ...
                                            <div key={u.id} className="p-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer" onClick={() => { setApprovalTarget(u); setShowApprovalModal(true); }}>
                                                <div>
                                                    <h4 className="font-bold text-sm dark:text-white flex items-center gap-2">
                                                        {currentLang === 'hi' ? (u.name_hi || u.name) : u.name}
                                                    </h4>
                                                    <p className="text-xs text-gray-500">{currentLang === 'hi' ? (DESIGNATIONS_HI[u.designation] || u.designation) : u.designation}</p>
                                                </div>
                    ...
                    <div className="animate-fade-in space-y-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700 relative">
                            ...
                            <div className="px-8 relative">
                                <div className="absolute -top-12 md:-top-20">
                                    <div className="relative inline-block">
                                        <img src={currentUser.photoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.name)}&background=random`} className="w-24 h-24 md:w-40 md:h-40 rounded-full border-4 border-white dark:border-gray-800 shadow-lg object-cover bg-gray-200" alt={currentUser.name} crossOrigin="anonymous" />
                                        {currentUser.badge && (
                                            <div className="absolute bottom-2 right-2 bg-white dark:bg-gray-900 rounded-full p-1.5 shadow-md z-10">
                            ...
                            <div className="mt-14 md:mt-24 px-4 md:px-8 pb-6 md:pb-10">
                                <div>
                                    <h2 className="text-xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-2 flex-wrap">
                                        {currentLang === 'hi' ? (currentUser.name_hi || currentUser.name) : currentUser.name}
                                    </h2>
                                    <p className="text-ljp-secondary dark:text-blue-400 font-bold text-base md:text-lg break-words">{currentLang === 'hi' ? (DESIGNATIONS_HI[currentUser.designation] || currentUser.designation) : currentUser.designation}</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 mt-5 md:mt-8">
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                        <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">{t.targetDistrict}</label>
                                        <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words"><MapPin size={20} className="mr-2 text-ljp-accent shrink-0" /> {currentLang === 'hi' ? (DISTRICTS_HI[currentUser.district] || currentUser.district) : currentUser.district}</p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                        <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Mobile</label>
                                        <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words"><Phone size={20} className="mr-2 text-ljp-accent shrink-0" /> {currentUser.mobile}</p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                        <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Area / Jurisdiction</label>
                                        <p className="font-semibold flex items-center text-gray-700 dark:text-gray-200 text-sm md:text-lg break-words">
                                            <LandPlot size={20} className="mr-2 text-ljp-accent shrink-0" />
                                            {currentUser.jurisdiction ? (currentLang === 'hi' ? (currentUser.jurisdiction_hi || currentUser.jurisdiction) : currentUser.jurisdiction) : 'Not Specified'}
                                        </p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700">
                                        <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Status</label>
                                        <p className="font-bold text-green-600 flex items-center text-sm md:text-lg">
                                            <CheckCircle size={20} className="mr-2 shrink-0" /> {t.activeMember}
                                        </p>
                                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-3xl p-6 relative z-10 shadow-2xl animate-fade-in">
                        ...
                        <form onSubmit={handleCreateMeeting} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <input type="date" value={newMeeting.date} onChange={e => setNewMeeting({ ...newMeeting, date: e.target.value })} onKeyDown={e => e.preventDefault()} onClick={(e) => (e.target as HTMLInputElement).showPicker && (e.target as HTMLInputElement).showPicker()} className="bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm dark:text-white cursor-pointer" required />
                                <input type="time" value={newMeeting.time} onChange={e => setNewMeeting({ ...newMeeting, time: e.target.value })} onKeyDown={e => e.preventDefault()} onClick={(e) => (e.target as HTMLInputElement).showPicker && (e.target as HTMLInputElement).showPicker()} className="bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm dark:text-white cursor-pointer" required />
                            </div>
                            ...
                            <div className="flex gap-3 mt-4">
                                <button type="button" onClick={() => setShowCreateMeetingModal(false)} className="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-3 rounded-xl font-bold">{t.cancel}</button>
                                <button type="submit" className="flex-[2] bg-ljp-secondary text-white py-3 rounded-xl font-bold hover:brightness-110">
                                    {t.schedule}
                                </button>
                            </div>
                        </form>
                        ...
                        <div className="space-y-4">
                            <div className="relative">
                                ...
                                <button
                                    ...
                                    disabled={isEnhancing || !newNoticeContent || enhanceCount >= 3}
                                ...
                                >
                                    ...
                                    {enhanceCount >= 3 ? "Limit Reached" : t.enhanceWithAI}
                                </button>
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-3xl p-6 relative z-10 shadow-2xl animate-fade-in">
                        ...
                        <form onSubmit={handleEditProfileSubmit} className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                <input type="text" placeholder={t.namePlaceholder} value={editProfileData.name}
                                    onChange={e => setEditProfileData({ ...editProfileData, name: e.target.value })}
                                    onBlur={() => handleTransliterateEditProfile('name', 'en')}
                                    className="w-full flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm dark:text-white" />
                                <input type="text" placeholder="नाम (Hindi)" value={editProfileData.name_hi}
                                    onChange={e => setEditProfileData({ ...editProfileData, name_hi: e.target.value })}
                                    onBlur={() => handleTransliterateEditProfile('name', 'hi')}
                                    className="w-full flex-1 bg-blue-50/50 dark:bg-gray-800 border-none rounded-xl p-3 text-sm dark:text-white" />
                            </div>

                            <div className="flex gap-2">
                                <input type="text" placeholder={t.fatherPlaceholder} value={editProfileData.fatherName}
                                    onChange={e => setEditProfileData({ ...editProfileData, fatherName: e.target.value })}
                                    onBlur={() => handleTransliterateEditProfile('fatherName', 'en')}
                                    className="w-full flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-3 text-sm dark:text-white" />
                                <input type="text" placeholder="पिता का नाम" value={editProfileData.fatherName_hi}
                                    onChange={e => setEditProfileData({ ...editProfileData, fatherName_hi: e.target.value })}
                                    onBlur={() => handleTransliterateEditProfile('fatherName', 'hi')}
                                    className="w-full flex-1 bg-blue-50/50 dark:bg-gray-800 border-none rounded-xl p-3 text-sm dark:text-white" />
                            </div>
                ...
                >
                    ...
                    <div className="border-b-4 border-ljp-secondary pb-6 mb-8 flex items-center justify-between">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-sm font-bold text-gray-900">Ref: LJP/SC/{downloadTargetNotice?.id.substring(0, 6).toUpperCase()}</p>
                            <p className="text-sm font-bold text-gray-900">Date: {downloadTargetNotice ? new Date(downloadTargetNotice.timestamp).toLocaleDateString() : new Date().toLocaleDateString()}</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-20 pt-12 border-t border-gray-100 flex justify-between items-end">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-2xl font-black text-gray-900 uppercase">{downloadTargetNotice?.userName}</p>
                            <p className="text-ljp-secondary font-bold text-lg">{downloadTargetNotice?.userDesignation}</p>
            ...
            <div className="fixed top-0 left-0 -z-[9999] opacity-0 pointer-events-none overflow-hidden">
                ...
                >
                    ...
                    <div className="mt-8 flex justify-center">
                        <div className="relative">
                            <div className="w-32 h-32 rounded-2xl border-4 border-ljp-secondary overflow-hidden shadow-2xl bg-gray-800">
                                <img src={currentUser.photoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.name)}&background=random&size=256`} crossOrigin="anonymous" className="w-full h-full object-cover" alt="ID" />
                            </div>
                            {currentUser.badge && (
                    ...
                    <div className="mt-6 px-8 text-center">
                        <h2 className="text-white font-bold text-2xl mb-1">{currentLang === 'hi' ? (currentUser.name_hi || currentUser.name) : currentUser.name}</h2>
                        <p className="text-ljp-secondary font-bold text-sm uppercase tracking-wider">{currentLang === 'hi' ? (DESIGNATIONS_HI[currentUser.designation] || currentUser.designation) : currentUser.designation}</p>
                        <div className="mt-4 inline-block px-3 py-1 bg-white/10 rounded-full border border-white/10">
                            <p className="text-white/60 text-[10px] font-bold uppercase">{currentLang === 'hi' ? (DISTRICTS_HI[currentUser.district] || currentUser.district) : currentUser.district} District</p>
                        </div>
                    </div>
